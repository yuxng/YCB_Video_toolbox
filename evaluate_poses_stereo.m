function evaluate_poses_stereo
...
% save results
distances_sys = zeros(100000, 6);
distances_non = zeros(100000, 6);
errors_rotation = zeros(100000, 6); 
errors_translation = zeros(100000, 6);
...
for seq_id = 48:59
    % read pair file
    filename = fullfile(opt.root, sprintf('pairs/%04d/pairs.txt', seq_id));
    index = load(filename);
    index(index == 0) = 1;
    ...
    for i = 1:size(index, 1)
        ...
        % load stereo results
        filename = sprintf('results_stereo_PoseCNN/%04d/%04d.mat', seq_id, i);
        object = load(filename);
        
        for k = 1:size(index, 2)
            
            if k == 1
                result_stereo = object.result_left;
            else
                result_stereo = object.result_right;
            end
            
            % load PoseCNN result
            str = sprintf('%04d/%06d', seq_id, index(i, k));
            im = find(strcmp(str, C{1}) == 1);
            filename = sprintf('results_PoseCNN/%04d.mat', im - 1);
            result = load(filename);
            
            % load 3D coordinate regression result
            filename = sprintf('results_3DCoordinate/%04d.mat', im - 1);
            result_3DCoordinate = load(filename);
    
            % load gt poses
            filename = fullfile(opt.root, 'data', sprintf('%04d/%06d-meta.mat', seq_id, index(i, k)));
            disp(filename);
            gt = load(filename);
            ...
            for j = 1:numel(gt.cls_indexes)
                ...
                results_frame_id(count) = index(i, k);
                ...
                % stereo result
                roi_index = find(result_stereo.rois(:, 2) == cls_index);
                if isempty(roi_index) == 0
                    RT = zeros(3, 4);
                    RT(1:3, 1:3) = quat2rotm(result_stereo.poses(roi_index, 1:4));
                    RT(:, 4) = result_stereo.poses(roi_index, 5:7);
                ...
                if isempty(roi_index) == 0
                    ...
                    distances_sys(count, 6) = adi(RT, RT_gt, models{cls_index}');
                    distances_non(count, 6) = add(RT, RT_gt, models{cls_index}');
                    errors_rotation(count, 6) = re(RT(1:3, 1:3), RT_gt(1:3, 1:3));
                    errors_translation(count, 6) = te(RT(:, 4), RT_gt(:, 4));                    
                else
                    distances_sys(count, 6) = inf;
                    distances_non(count, 6) = inf;
                    errors_rotation(count, 6) = inf;
                    errors_translation(count, 6) = inf;
                end                
...
save('results_stereo.mat', 'distances_sys', 'distances_non', 'errors_rotation', 'errors_translation',...
